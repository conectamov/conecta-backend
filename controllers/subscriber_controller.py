...
@api.validate(
    ...
    tags=["subscriber"],
    resp=Response(
        HTTP_200=DefaultResponse, HTTP_400=DefaultResponse, HTTP_500=DefaultResponse
    ),
)
def subscribe():
    """
    ...
    data = request.json
    try:
        Config.mail_client.subscribers.create(
            data["email"], fields={"name": data["name"]}
        )
    except:
        return {
            "msg": "Failed subscribing: the recipient address may be invalid or the mail service is unreachable."
        }, 500
